                    ...
                    return (
                      ...
                      >
                        <div 
                          ...
                          className="group bg-white dark:bg-slate-800/90 hover:bg-slate-50 dark:hover:bg-slate-750/80 border border-slate-100 dark:border-white/5 rounded-2xl p-4 flex items-center justify-between shadow-sm hover:shadow-md hover:-translate-y-[1px] transition-all duration-200 ease-out w-full cursor-pointer pointer-events-auto relative overflow-hidden"
                        >
        ...
        <>
          {(() => {
            ...
            if (categoryUpper === 'USER RENEW') {
              ...
              const matchedUser = users.find(u => u.id === selectedTransaction.userId);
              detailsList.push({
                label: language === 'bn' ? 'কর্মচারী নাম' : 'Employee Name',
                value: matchedUser?.name || selectedTransaction.details?.userName || 'N/A'
              });
              if (matchedUser?.mobileNumber) {
                detailsList.push({
                  label: language === 'bn' ? 'মোবাইল নম্বর' : 'Mobile Number',
                  value: matchedUser.mobileNumber
                });
              }
              if (selectedTransaction.month) {
                detailsList.push({
                  label: language === 'bn' ? 'মাস' : 'Month',
            ...
            return (
              <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 bg-black/50 backdrop-blur-sm animate-fade-in">
                ...
                />
                <div 
                  className="bg-theme-card w-full max-w-md h-[520px] rounded-[24px] overflow-hidden shadow-[0_24px_60px_rgba(0,0,0,0.15)] dark:shadow-[0_24px_60px_rgba(0,0,0,0.4)] flex flex-col relative z-10 border border-black/5 dark:border-white/10 animate-scale-in"
                >
                  {/* Category Header */}
